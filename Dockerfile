...
ENTRYPOINT ["java", "-Dio.opentelemetry.javaagent.slf4j.simpleLogger.defaultLogLevel=debug", "-javaagent:/javaagent.jar" ,"-cp", "/app/resources:/app/classes:/app/libs/*", "org.hypertrace.core.serviceframework.PlatformServiceLauncher"]

# docker build -f Dockerfile . -t pavolloffay/ht-entity-service-agent-enabled:2
# docker run --rm -it -p 50061:50061 -p 50062:50062 pavolloffay/ht-entity-service-agent-enabled:2
